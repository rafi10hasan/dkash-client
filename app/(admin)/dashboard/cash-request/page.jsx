...
export default function CashRequest() {
  ...
  const fetchCashInAccounts = async () => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/transaction/cash-requests`, {
      method: "GET",
      headers: {
  ...
  const handleStatusChanged = async (userId,statusMsg)=>{
       let statusData = {
        status:statusMsg,
       }

       try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/transaction/cash-request-success/${userId}`, {
          method: "PUT",
          headers: {
             Authorization: `Bearer ${session?.accessToken}`,
            "Content-Type": "application/json",
                    ...
                    {accounts?.map((account) => (
                      <tr key={account._id} className="border-b dark:border-gray-700">
                        ...
                        <td className="px-4 py-3">
                                ...
                                <Button disabled={account.status==="approved"} onClick={()=>handleStatusChanged(account._id,"approved")} className={`${account.status==="approved" ? "bg-gray-600 cursor-not-allowed" :"bg-green-600 cursor-pointer"}`}>
